...
if(isset($_POST['signIn'])){
	...
	if(!isset($_POST['email']) || !mailVal($_POST['email'])){
		...
		if(!isset($_POST['pass']) || empty($_POST['pass'])){
			...
			// $LTH = $myconn->prepare("SELECT ID,OID,pass,private_key,isActive,mail,last_login FROM ". db_table_pref ."users WHERE ID=1") or die(mysqli_error($myconn));
...
if(isset($_POST['sendPW'])){
	if(DEMO_MODE){
		...
		if(!isset($_POST['pw_rem']) || !mailVal($_POST['pw_rem'])){
			...
			$opUser = $db->where('mail=?',array($_POST['pw_rem']))->getOne('users');
			...
			if($db->count<1){
				...
				# Start
					...
					$mailBody = '<p>Hello '. $opUser['real_name'] .',</p>';
					...
					$rcMail = showIn($opUser['mail'],'page');
					$rcName = showIn($opUser['real_name'],'page');
					...
					$sendMail->sysSubInitPMail();
				
					if($sendMail->sendPos){
						# Change Password
						
						$db->where('ID=?',array($opUser['ID']))->update('users',array('pass'=>$newPassEnc));

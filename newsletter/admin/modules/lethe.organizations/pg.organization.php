...
if(LETHE_AUTH_MODE!=2){
	$seoid = $myconn->query("select ID,private_key from ". db_table_pref ."organizations where ID=". LETHE_AUTH_ORG_ID ."");
	$seoidr = $seoid->fetch_assoc();
	if(!defined('LETHE_AUTH_ORG_PRIVATE')){
		define('LETHE_AUTH_ORG_PRIVATE',$seoidr['private_key']);
	}

...
if (strcmp ($res, "VERIFIED") == 0) {
	...
	$cust_email =  $_MYVAR['custom_email'];
    $cust_name =  $_MYVAR['custom_name'];
	$item_value = $_MYVAR['no_shipping'];
	...
	if($isPaymentCompleted) {
	    $param_type = "sssdss";
	    $param_value_array = array($item_number, $item_value, $payment_status, $payment_amount, $payment_currency, $txn_id);
	    ...
	    error_log(date('[Y-m-d H:i e] '). "Vdddddddddddderified IPN: $req ". PHP_EOL, 3, LOG_FILE);
	       ...
	       $update_fields = array('no_of_account'=>$item_value);
	       $where_condition = array('Email'=>$cust_email);                                                  
           $db->update("UserTable",$update_fields,$where_condition);
	} 

            ...
            success: function (data) {
                ...
                if ( data ) {
                    ...
                    for (var u = 0; u < data.users.length; u++) {
                        ...
                        // Get the edit button
                        var edit = '<button type="button" data-user="' + data.users[u].user_id + '" class="btn btn-edit pull-right user-edit"><i class="fa fa-pencil"></i></button>';
                        
                        // Verify if admin is in the statistics page 
                        if ( jQuery('.widget-box').length > 0 ) {
                            
                            edit = '<button type="button" data-user="' + data.users[u].user_id + '" class="btn btn-edit pull-right user-statistics"><i class="fa fa-bar-chart"></i></button>';
    ...
    jQuery(document).on('click', '.confirm .yes', function (e) {
        ...
        jQuery.ajax({
            ...
            error: function (data, jqXHR, textStatus) {
                console.log('Request failed: ' + textStatus);
                jQuery('.alert-msg').show();
                jQuery('.alert-msg').html('<p class="merror">'+translation.ma142+'</p>');
                jQuery('.merror').fadeIn(1000).delay(2000).fadeOut(1000, function () {
                    jQuery('.merror').remove();
                    jQuery('.alert-msg').hide();
                });
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                if (data.msg) {
                    ...
                    var gettime = calculate_time(data.msg.date, data.msg.current);

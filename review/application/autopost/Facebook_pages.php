...
class Facebook_pages implements Autopost {
    ...
    public function connect() {
        ...
        if ( get_option('facebook_pages_user_api_key') ) {   
            ...
            if( !isset($_SESSION['app_id']) ) {
                ...
                echo get_instance()->ecl('Social_login')->content('App ID', 'App Secret', 'Connect', $this->get_info(), 'facebook_pages', $this->CI->lang->line('mu208'));
                exit();
        ...
        // Get redirect url
        $loginUrl = $helper->getLoginUrl(get_instance()->config->base_url() . 'user/callback/facebook_pages', $permissions);
    ...
    public function save($token = null) {
        ...
        // Get user data
        $response = json_decode(get('https://graph.facebook.com/me/accounts?limit=500&access_token=' . $access_token[0]));
        ...
        if (@$response->data[0]->access_token) {
            ...
            if (property_exists($response->data[0], 'id')) {
                    ...
                    for ($m = 0; $m < count($response->data); $m++) {
                        ...
                        if ( $acci ) {
                            ...
                            if ( !isset($_SESSION['app_id']) ) {
                                ...
                                if ( $this->CI->networks->add_network('facebook_pages', $response->data[$m]->id, $access_token[0], $user_id, $expires, $response->data[$m]->name, '', $response->data[$m]->access_token) ) {
                                ...
                                if ( $this->CI->networks->add_network('facebook_pages', $response->data[$m]->id, $access_token[0], $user_id, $expires, $response->data[$m]->name, '', $response->data[$m]->access_token, $_SESSION['app_id'], $_SESSION['app_secret']) ) {
                ...
                unset($_SESSION['app_id']);
                unset($_SESSION['app_secret']);
                $this->CI->session->set_flashdata('deleted', display_mess(82));
            }
        }
    }
    ...
    public function post($args, $user_id = null) {
        ...
        try {
            ...
            // Set access token
            $response = @json_decode(file_get_contents('https://graph.facebook.com/me/accounts?limit=500&access_token=' . $user_details[0]->token));
            ...
            if ( @$response->data ) {
                ...
                foreach ( $response->data as $page ) {
                    ...
                    if ( $user_details[0]->net_id == $page->id ) {
                ...
                if ( $token ) {
                    ...
                    if ( $args['img'] ) {
                        ...
                        $postu = $this->fb->post('/' . $user_details[0]->net_id . '/photos', ['url' => $args['img'], 'published' => FALSE], $token);
                        ...
                        if ( @$postu->getDecodedBody() ) {
                            ...
                            if ( @$mo['id'] ) {
                                ...
                                $post = $this->fb->post('/' . $user_details[0]->net_id . '/feed', ['message' => $post, 'attached_media[0]' => '{"media_fbid":"'.$mo['id'].'"}'], $token);
    ...
    public function preview($args) {
        ...
        return (object) [
                    'name' => 'facebook_pages',
                    'icon' => '<button type="button" class="btn btn-network-f"><i class="fa fa-facebook-official"></i><span data-network="facebook_pages"><i class="fa fa-times"></i></span></button>',
                    'head' => '<li><a href="#facebook" data-toggle="tab"><i class="fa fa-facebook-official" aria-hidden="true"></i></a></li>',
                    'content' => '<div class="tab-pane" id="facebook_pages">
                                  <div class="facebook_pages forall">
                                                      ' . $url . '
                                                  </div>
                                          </div>'];
    }

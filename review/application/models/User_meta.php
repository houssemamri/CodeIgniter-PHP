...
class User_meta extends CI_MODEL {
     ...
     * @return boolean true or false
     */    
    public function set_post_number( $user_id ) {
        
        $this->db->select('meta_name,meta_value');
        $this->db->from($this->table);
        $this->db->where(['user_id' => $user_id,'meta_name' => 'published_posts']);
        $query = $this->db->get();
        ...
        if ( $query->num_rows() > 0 ) {
            ...
            $decode = unserialize($result[0]->meta_value);
            
            if ( $decode['date'] == date('Y-m') ) {
                
                $posts = $decode['posts'];
                $posts++;
                $date = serialize(['date'=>date('Y-m'),'posts'=>$posts]);
                $data = ['user_id'=>$user_id, 'meta_name'=>'published_posts', 'meta_value'=>$date];
                $this->db->where(['user_id' => $user_id,'meta_name' => 'published_posts']);
                $this->db->update($this->table, $data);
                ...
                $date = serialize(['date'=>date('Y-m'),'posts'=>1]);
                $data = ['user_id'=>$user_id, 'meta_name'=>'published_posts', 'meta_value'=>$date];
                $this->db->where(['user_id' => $user_id,'meta_name' => 'published_posts']);
                $this->db->update($this->table, $data);
    ...
    /**
     * The public method get_post_number gets number of published posts
     *
     * @param integer $user_id contains user_id
     * 
     * @return integer with user's posts or false
     */    
    public function get_post_number( $user_id ) {

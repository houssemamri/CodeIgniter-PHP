...
class Posts extends CI_MODEL {
    ...
    public function get_post( $user_id, $msgId ) {
        ...
        if ( $query->num_rows() > 0 ) {
            ...
            return ['post_id' => $result[0]->post_id, 'user_id' => $result[0]->user_id, 'body' => $result[0]->body, 'title' => $result[0]->title, 'category' => $result[0]->category, 'url' => $result[0]->url, 'img' => $result[0]->img, 'video' => $result[0]->video, 'status' => $result[0]->status, 'sent' => $sent, 'time' => $result[0]->sent_time, 'current' => time(), 'parent' => $result[0]->parent];
    ...
    public function get_posts( $user_id, $start, $limit, $key = NULL ) {
        ...
        if ( $query->num_rows() > 0 ) {
            ...
            foreach ( $results as $result ) {
                ...
                $array[] = (object)[ 'post_id' => $result->post_id, 'body' => $result->body, 'title' => $result->title, 'url' => $result->url, 'video' => $result->video, 'img' => $result->img, 'sent_time' => $result->sent_time, 'status' => $result->status, 'history' => $this->sentnetworkmess($result->post_id) ];
    ...
    public function delete_posts( $user_id ) {
        ...
        $sql = 'DELETE m,a FROM posts m JOIN posts_meta a ON m.post_id = a.post_id WHERE m.user_id=?';
    ...
    protected function lacu() {
        ...
        $rd = get_option('monitoris_limit') ? get_option('monitoris_limit') : 1;
        return $rd * 6400;

 ...
 * PHP Version 5.6
 *
 * Team file contains the Team Model
 *
 * @category Social
 * @package  Midrub
...
class Team extends CI_MODEL {
     ...
     */
    public function save_member( $user_id, $username, $password ) {
        ...
        $data = ['user_id' => $user_id, 'member_username' => $username, 'member_password' => $this->bcrypt->hash_password($password), 'date_joined' => date('Y-m-d H:i:s')];
     ...
     * @return boolean true or false
     */
    public function update_member( $user_id, $member_id, $password ) {
        
        $this->db->where( [ 'member_id' => $member_id, 'user_id' => $user_id ] );
        $this->db->update( $this->table, ['member_password' => $this->bcrypt->hash_password($password)] );
     ...
     * @return object with members or false
     */
    public function get_member( $user_id, $member_id ) {
        
        $this->db->select('member_id, member_username, UNIX_TIMESTAMP(date_joined) AS date_joined,UNIX_TIMESTAMP(last_access) AS last_access');
        $this->db->from($this->table);
        $this->db->where( [ 'member_id' => $member_id, 'user_id' => $user_id ] );
        $query = $this->db->get();
    ...
    public function delete_member( $user_id, $member_id ) {
        ...
        $this->db->delete( $this->table, [ 'member_id' => $member_id, 'user_id' => $user_id ] );

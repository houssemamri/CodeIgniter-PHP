jQuery(document).ready(function() {
   ...
   var url=jQuery('#base_url').val();
            ...
            success: function (data) {
                if(data) {
                    ...
                    for(var g = 0; g < data.length; g++) {
                        questions += '<div class="col-lg-12 single-question"><div class="mm-single-result"><h3>' + data[g].question + '</h3><p>' + data[g].response + '</p></div></div>';
                    }
    ...
    function get_tickets(page) {
        jQuery.ajax({
            url: url + 'user/get-tickets/' + page + '/tickets',
            dataType: 'json',
            type: 'GET',
            success: function (data) {
                if (data) {
                    ...
                    for (var u = 0; u < data.tickets.length; u++) {
                        var status = (data.tickets[u].status == 1)?'<span class="label label-success">' + translation.mi34 + '</span>':(data.tickets[u].status == 2)?'<span class="label label-primary">' + translation.mi31 + '</span>':'<span class="label label-default">' + translation.mi32 + '</span>';
                        alltickets += '<tr><td width="80%"><h3>' + data.tickets[u].subject + ' </h3>' + status + '</td><td width="10%"><a href="' + url + 'user/get-ticket/' + data.tickets[u].ticket_id + '" class="btn btn-primary btn-xs"><i class="fa fa-search"></i></a></td><td width="10%"><button class="btn btn-danger btn-xs delete-ticket" data-id="' + data.tickets[u].ticket_id + '">' + translation.mm133 + '</button></td></tr>';
                    }

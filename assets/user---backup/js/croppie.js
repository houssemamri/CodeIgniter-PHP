...
(function (root, factory) {
    ...
    jQuery('.crop-img').on('click', function () {
        var imo = jQuery('.img>a').attr('href');
        jQuery('.show-image').empty();
        $uploadCrop = jQuery('.show-image').croppie({
            enableExif: true,
            viewport: {
                width: 500,
                height: 500,
        ...
        jQuery('.cr-image').removeAttr('style');
    });
    ...
    function _data(data)
    {
        if(data)
        {
            jQuery('.img>a').attr('href',data);
            jQuery('.img>a').text(data);
            jQuery('.aimg').val(data);
        }
    }
    ...
    function _result(options) {
        ...
        if (size === 'viewport') {
            data.outputWidth = 600;
            data.outputHeight = 600;
    ...
    function pop(result) {
        ...
        var encode = btoa(result.src);
        encode = encode.replace('/', '-');
        var led = encode.replace(/=/g, '');
        var data = {'im': led, 'csrf_test_name': name};
        jQuery.ajax({
            url: jQuery('.navbar-brand').attr('href') + 'user/get-img',
            dataType: 'json',
            type: 'POST',
            data: data,
    ...
    jQuery('.crop-the-im').on('click', function (ev) {

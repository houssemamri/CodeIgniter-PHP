...
$PNG_WEB_DIR = '../qrcode/'.$outdir.'/';

require "../qrcode/lib/phpqrcode.php";
...
if ($output_data) {
    ...
    if ($optionlogo && $optionlogo !== 'none') {
		$text = $optionlogo;
		 $text1 = $PNG_WEB_DIR.$finalpng;

$arr = explode("/", $text);
$arr1 = explode("/", $text1);
//shifting array three times, increase if necessary
array_shift($arr);
array_shift($arr1);array_shift($arr1);
//implode and return
  $front=implode("/", $arr);
  $back=implode("/", $arr1);

		
        $mergedimage = mergeImages($back, $front, false);
    }
	
    if ($mergedimage) {
		
        $placeholder = '/qrcode/'.$mergedimage;
    } else {
...
$user_id=$_SESSION['user_id'];
 $sql="INSERT INTO user_qrcode(user_id,section,data,png,svg,eps,placeholder,optionlogo,otp) VALUES ('" . $user_id . "','" . $getsection . "','" . $output_data . "','" . $finalpng . "','" . $finalsvg . "','".$finaleps."','".$placeholder."','".$optionlogo."','".$otp."')";
 $conn->query($sql);    

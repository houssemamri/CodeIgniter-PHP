...
//print_r($_POST);die("here");
   ...
   while($result = mysqli_fetch_assoc($queryGroupId)){
	...
	if($mail_success){
		/*$content = mysqli_real_escape_string($conn,trim($body));
		echo $content;*/
		
		
		$db = new Db();
		
		$sqlEmailLog = $db->insertEmailLog($_SESSION['user_id'],$lists_id[$l],$choose_temp,$to1,$subject,$content,date('Y-m-d'),date('H:i:s'));
...
<body>
	...
	<!--<div class="header drag_mail">-->
		<?php include("../nav_drag_page.php");?>
...
<?php } ;?>						  
				...
				<div class="row">
					<div class="col-md-12" id="subject">
						...
						<input class="form-control" required=""  name="subject" value="" type="text" />
					</div>
				</div> <br>
...
function testEmailFunction(){
    ...
    function myFunction(){
		  var emailBody = document.querySelector(".content").innerHTML;
								...
								$.ajax({
										...
										success: function(data) {
											...
											for (var i = 0; i < data.blocks.length; i++) {
												_content='';
												_content += '<div class="sortable-row" choose_temp="'+data.template.id+'">' +
																'<div class="sortable-row-container">' +
																' <div class="sortable-row-actions">';

...
$(document).ready(function () {
    ...
    $(document).on('change', '.social-input', function (e) {
        ...
        $('#' + $('#path').val()).find($('#selector').val() + ' a.' + name).attr('href', 'https://' + $(this).val()).attr('target', '_blank');
    });
    ...
    $("#save").click(function () {
        ...
        if(name.length>0) {
            if($('#name').parent().hasClass('has-error')) $('#name').parent().removeClass('has-error');
            $.post(path + '/save.php', {html: save.html(), data: save.data(), name: name,id:id}, function (data) {
                $('#templateid').val(data);
                toastr.success('Saved!');
            }, 'html');
        }else{
            if(!$('#name').parent().hasClass('has-error')) $('#name').parent().addClass('has-error');
            toastr.error('Please enter a name for your template!')

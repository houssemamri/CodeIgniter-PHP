 ...
 * @link https://www.midrub.com/
 */
class Google_mybusiness implements Autopost {
    ...
    public function __construct() {
     ...
     $this->clientId = get_option('google_mybusiness_client_id');
        
        // Get the Google's client_secret
        $this->clientSecret = get_option('google_mybusiness_client_secret');
        
        // Get the Google's api key
        $this->apiKey = get_option('google_mybusiness_api_key');
        
        // Get the Google's application name
        $appName = get_option('google_mybusiness_application_name');
        ...
        $scriptUri = base_url() . 'user/callback/google_mybusiness';
        ...
        $this->client->setScopes( array(
            "https://www.googleapis.com/auth/plus.business.manage https://www.googleapis.com/auth/userinfo.profile" ) );
    ...
    public function save( $token = null ) {
        ...
        if ( $this->CI->input->get('code', TRUE) ) {
            ...
            $res=$this->client->authenticate( $this->CI->input->get('code', TRUE) );
            ...
            if ( @$token->access_token ) {
                ...
                // Set expires time
                $expires_in = '';
                // echo 'https://www.googleapis.com/oauth2/v3/userinfo?access_token=' . $token;
				// die;
                $curl = curl_init();
                ...
                if ( $udata ) {
                    ...
                    if ( @$udecode->sub ) {
                     ...
                     $user_id = $this->CI->user->get_user_id_by_username($this->CI->session->userdata ['username']);
                        ...
                        if ( !$this->CI->networks->get_network_data('google_mybusiness', $user_id, $udecode->sub) ) {
                            
                            // Save account
                            $this->CI->networks->add_network('google_mybusiness', $udecode->sub, $token, $user_id, $expires_in, $udecode->name, $udecode->picture, $refresh);
        ...
        if ( $con ) {
            ...
            if ( $con [0]->secret ) {
                ...
                try {
                    ...
                    $post_data = ['object' => ['originalContent' => $post . ' ' . $args['url']], 'access' => ['items' => [['type' => 'domain']], 'domainRestricted' => true]];
    ...
    public function preview($args) {
        ...
        return (object) [
                    ...
                    'icon' => '<button type="button" class="btn btn-network-y"><i class="fa fa-google"></i><span data-network="google_plus"><i class="fa fa-times"></i></span></button>',
                    'head' => '<li class="google_mybusiness"><a href="#google_mybusiness" data-toggle="tab"><i class="fa fa-google"></i></li>',
                    'content' => '<div class="tab-pane" id="google_mybusiness">
                                  <div class="google_mybusiness forall"><div>
                                      <p class="prevtext"></p>
                                  </div>
                                  </div>
                                  </div>'

...
class User extends CI_MODEL {
     ...
     * @return boolean true or false
     */
    public function signup( $username, $email, $password, $pstatus = null, $prole = null ) {
        ...
        $data = ['username' => strtolower($username), 'email' => strtolower($email), 'password' => $this->bcrypt->hash_password($password), 'role' => $role, 'status' => $status, 'date_joined' => date('Y-m-d H:i:s'), 'ip_address' => $ip];
     ...
     * @return boolean true or false
     */
    public function updateuser( $user_id, $email, $password, $role=NULL, $status=NULL, $plan=NULL, $proxy=NULL ) {
        ...
        if ( $user_id ) {
            ...
            if ( $role == null ) {
                ...
                if ( $password ) {
                    ...
                    $this->db->update($this->table, ['email' => $email, 'password' => $this->bcrypt->hash_password($password)]);
                    
                } else {
                    
                    $this->db->update($this->table, ['email' => $email]);
                ...
                if ( $password ) {
                    ...
                    $this->db->update($this->table, ['email' => $email, 'password' => $this->bcrypt->hash_password($password), 'role' => $role, 'status' => $status]);
                    
                } else {
                    
                    $this->db->update($this->table, ['email' => $email, 'role' => $role, 'status' => $status]);
    ...
    public function check_role_by_email( $email ) {
    ...
    public function check_status_by_username( $username ) {
     ...
     public function check_status_by_email( $email ) {
    ...
    public function get_user_info( $user_id ) {
        ...
        $this->db->select('user_id,username,email,role,status,date_joined');
        $this->db->from($this->table);
        $this->db->where('user_id', $user_id);
        $query = $this->db->get();
        ...
        if ( $query->num_rows() == 1 ) {
            ...
            return ['user_id'=>$result[0]->user_id,'email'=>$result[0]->email,'username'=>$result[0]->username,'role'=>$result[0]->role,'status'=>$result[0]->status,'proxy'=>$proxy,'date'=>strtotime($result[0]->date_joined),'current'=>time(),'plan'=>$plan];

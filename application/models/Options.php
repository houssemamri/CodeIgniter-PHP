...
class Options extends CI_MODEL {
     ...
     */
    public function add_option_value( $option, $value ) {
        
        $this->db->select('*');
        $this->db->from($this->table);
        $this->db->where(['option_key'=>$option]);
        $query = $this->db->get();
        $value = str_replace('empty-option', '', $value);
        
        if ( $query->num_rows() > 0 ) {
            
            $this->db->set(['option_key'=>$option,'option_value'=>trim($value)]);
            $this->db->where(['option_key'=>$option]);
            ...
            $data = ['option_key'=>$option,'option_value'=>trim($value)];
            $this->db->insert($this->table, $data);
            return true;

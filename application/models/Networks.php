...
class Networks extends CI_MODEL {
    ...
    public function add_network( $name, $net_id, $token, $user_id, $expires, $user_name, $user_avatar, $secret = null, $app_id = null, $app_secret = null ) {
        ...
        if ( ($app_id != NULL) && ($app_secret != NULL) ) {
            ...
            // Add new row
            $data = ['network_name' => strtolower($name), 'net_id' => $net_id, 'user_id' => $user_id, 'user_name' => $user_name, 'user_avatar' => $user_avatar, 'date' => date('Y-m-d h:i:s'), 'expires' => $expires, 'token' => $token, 'secret' => $secret, 'api_key' => $app_id, 'api_secret' => $app_secret];
            
        } else {
            
            // Add new row
            $data = ['network_name' => strtolower($name), 'net_id' => $net_id, 'user_id' => $user_id, 'user_name' => $user_name, 'user_avatar' => $user_avatar, 'date' => date('Y-m-d h:i:s'), 'expires' => $expires, 'token' => $token, 'secret' => $secret];
    ...
    public function get_networks($user_id) {
        ...
        $this->db->select('network_name,net_id,token,UNIX_TIMESTAMP(expires) AS expires');
        $this->db->from($this->table);
        ...
        if ( $query->num_rows() > 0 ) {
            ...
            foreach ( $result as $data ) {
                ...
                $new_array[] = ['network_name' => $data->network_name, 'net_id' => $data->net_id, 'token'=>$data->token,'expires' => $data->expires];
    ...
    public function get_network_accounts( $network ) {
        ...
        $this->db->select('network_id,net_id,network_name,user_name,user_avatar,token,secret,COUNT(network_name) AS num');
        $this->db->from($this->table);
        $this->db->where(['network_name'=>strtolower($network)]);
        $query = $this->db->get();

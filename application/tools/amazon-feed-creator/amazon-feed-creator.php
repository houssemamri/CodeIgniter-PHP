...
class Amazon_feed_creator implements Tools {
    ...
    public function check_info() {
        return (object) ['name' => 'Amazon RSS Creator', 'full_name' => 'Amazon RSS Creator', 'logo' => '<button class="btn-tool-icon btn btn-default btn-xs  pull-left" type="button"><i class="fa fa-wrench"></i></button>', 'slug' => 'amazon-feed-creator'];
    }
    ...
    public function page($args) {
        ...
        $content = '<p>'.get_instance()->lang->line('mu60').'</p>'.get_instance()->lang->line('mu119');
        $url = '';
        $title = '';
        $show = '';
        $CI =& get_instance();
        ...
        if ($CI->input->post()) {
            ...
            if ((@$curl == @$url) && (@$title != '')) {
                ...
                if(($all_rss?count($all_rss):0) < $rss_limit){
                    $response = $CI->rss->save_new_rss($args['user_id'], site_url('user/tools/amazon-feed-creator').'?rss-url='.base64_encode($url).'&tool='.$this->check_info()->slug, $title);
                } else {
                    $response = 0;
                }
                if ($response == 1) {
                                                ...
                                                <li>
                                                    ...
                                                    ' . form_open('user/tools/amazon-feed-creator') . '
                                                        <div class="input-group search">
                                                            <input type="text" name="feed-url" placeholder="'.get_instance()->lang->line('mu116').'" value="' . $url . '" class="form-control rss-url" required="true">

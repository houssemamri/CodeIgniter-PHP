...
class Views {
    ...
    public function single($param) {
        if ($param) {
            ...
            foreach ($param as $val) {
                ...
                if ($val->img) {
                    $img = '<div class="row">
                        <div class="col-lg-12">
                        <p><img src="' . $val->img . '" style="max-height:100%"></p>
                        </div></div>';
                }
                $video = '';
                if ($val->video) {
                    $video = '<div class="row">
                    <div class="col-lg-12">
                    <p><video controls="true" style="width:100%;height:300px"><source src="' . $val->video . '" type="video/mp4" /></video></p>
                    </div></div>';
                }
                $del = ' hide';
                $td = '';
                ...
                $data .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-lg-offset-3 single-activity" id="activity-' . $val->activity_id . '">
                            <div class="col-lg-12">
                                <div class="row">
                                ...
                                <div class="row will-del' . $del . '">
                                    <div class="col-md-12">
                                      <div class="sched-del">
                                        <div class="del-split del-danger"><i class="fa fa-bell-o" aria-hidden="true"></i></div>
        ...
        if (!@$data) {
            $data = '<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-lg-offset-3 single-activity"><div class="col-lg-12"><div class="row">'
                    . '<p align="center">' . $this->CI->lang->line('mt48') . '</p>'
                    . '</div></div></div>';
        }
    ...
    public function com($id, $permalink, $post) {
        ...
        return '<div class="comfo">
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="active"><a href="#comments-' . $id . '" aria-controls="comments-' . $id . '" role="tab" data-toggle="tab"><i class="fa fa-comments" aria-hidden="true"></i> ' . $this->CI->lang->line('mt55') . ' (' . $comments . ')</a>
                        </li>
                ...
                <div class="modal fade" id="addrep" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            ' . form_open('#', ['class' => 'add-reply']) . '
                                <div class="row">
                                    <div class="col-md-12">
                                        <textarea placeholder="' . $this->CI->lang->line('mt57') . '" class="msg"></textarea>
                                    </div>
                                    <div class="col-md-12">
                                        <button class="pull-right btn" type="submit">' . $this->CI->lang->line('mt51') . '</button>
                                    </div>
                                </div>
                            ' . form_close() . '
                            </div>
    ...
    private function footer() {
        return '<div class="modal fade bs-delete-post" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                    <div class="modal-dialog modal-sm" role="document">
                        <div class="modal-content">
                            ' . form_open('#', ['class' => 'set-date']) . '
                                <div class="input-group">
                                    <input type="text" class="form-control schedule-deletion time-schedule" placeholder="' . $this->CI->lang->line('mt58') . '" required>
                                    <span class="input-group-btn">
                                      <button class="btn btn-success btn-circle" type="submit">' . ucfirst($this->CI->lang->line('mm114')) . '</button>
                                    </span>                        
                                </div>
                            ' . form_close() . '
                        </div>
                ...
                <div class="modal fade bs-repost-post" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                    <div class="modal-dialog modal-sm" role="document">
                        <div class="modal-content">
                            ' . form_open('#', ['class' => 'shed-date']) . '
                                <div class="input-group">
                                    <input type="text" class="form-control schedule-rep time-schedule" placeholder="' . $this->CI->lang->line('mt58') . '" required>
                                    <span class="input-group-btn">
                                      <button class="btn btn-success btn-circle" type="submit">' . $this->CI->lang->line('mt59') . '</button>
                                    </span>                        
                                </div>
                            ' . form_close() . '
                        </div>
    ...
    public function show_pages($total, $current, $limit, $j) {
        if ($current > 1) {
            $bac = $current - 1;
            $pages = '<li><a href="' . base_url($j . $bac) . '">' . $this->CI->lang->line('mm129') . '</a></li>';
        } else {
        ...
        for ($p = $from; $p < $tot; $p++) {
            if ($p == $current) {
                ...
                $pages .= '<li><a href="' . base_url($j . $p) . '">' . $p . '</a></li>';
        ...
        if ($p == 1) {
            $pages .= '<li class="active"><a href="' . base_url($j . $p) . '">' . $p . '</a></li>';
        }
        $next = $current;
        $next++;
        if ($next < $tot) {
            return $pages . '<li><a href="' . base_url($j . $next) . '">' . $this->CI->lang->line('mm128') . '</a></li>';
        } else {
            return $pages . '<li class="pagehide"><a href="#">' . $this->CI->lang->line('mm128') . '</a></li>';
        }
    }
        ...
        if ($param) {
            foreach ($param as $par) {
                ...
                $data .= '<tr class="pt-' . $par->post_id . '">
                            <td>
                            <i class="fa fa-calendar-check-o" aria-hidden="true"></i> ' . $this->CI->lang->line('mt60') . ' ' . strip_tags(calculate_time($par->sent_time, time())) . ' ' . $this->CI->lang->line('mt61') . ' 
                                <div class="btn-group">
